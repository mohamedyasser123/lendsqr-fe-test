...
const { data, isLoading, isError, error, refetch } = useUser(id!);
    ...
    return (
      <div className="user-details-page">
        ...
        <div style={{ marginTop: '24px', marginBottom: '32px' }}>
          <div style={{ height: '28px', width: '200px', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', borderRadius: '4px' }} />
        </div>
        <div className="user-summary-card" style={{ minHeight: '200px', display: 'flex', alignItems: 'center', padding: '30px' }}>
          <div style={{ width: '100px', height: '100px', borderRadius: '50%', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', flexShrink: 0 }} />
          <div style={{ marginLeft: '20px', flexGrow: 1 }}>
            <div style={{ height: '24px', width: '150px', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', borderRadius: '4px', marginBottom: '12px' }} />
            <div style={{ height: '16px', width: '100px', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', borderRadius: '4px' }} />
          </div>
        </div>
        <div className="user-details-card" style={{ marginTop: '24px', padding: '30px', minHeight: '400px' }}>
          <div style={{ height: '20px', width: '150px', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', borderRadius: '4px', marginBottom: '32px' }} />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '30px 20px' }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ height: '12px', width: '80px', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', borderRadius: '4px' }} />
                <div style={{ height: '16px', width: '140px', background: 'linear-gradient(90deg, #f2f3f5 25%, #e6e8eb 37%, #f2f3f5 63%)', backgroundSize: '400% 100%', animation: 'skeleton-loading 1.4s ease infinite', borderRadius: '4px' }} />
              </div>
            ))}
          </div>
        </div>
  ...
  if (isError || !data) {
    // Detect 404 (user does not exist) vs other network errors
...
{activeTab === "general" && <GeneralDetailsTab user={data} />}
        ...
        {activeTab === 'bank-details' && (
          <div
            id="tabpanel-bank-details"
            role="tabpanel"
            aria-labelledby="tab-bank-details"
            className="details-section"
          >
            <p style={{ color: 'var(--color-secondary, #545F7D)', fontFamily: 'Work Sans, sans-serif', fontSize: '14px' }}>
        ...
        {activeTab === 'app-system' && (
          <div
            id="tabpanel-app-system"
            role="tabpanel"
            aria-labelledby="tab-app-system"
            className="details-section"
          >
            <p style={{ color: 'var(--color-secondary, #545F7D)', fontFamily: 'Work Sans, sans-serif', fontSize: '14px' }}>
